...
単なるシャッター操作に留まらず、撮影の瞬間にINDIサーバーから天体座標（RA/Dec）や気象データを取得し、画像メタデータと同期。**OrionFieldStack JSON Log Specification (v1.5.0)** に基づいた高精度な標準化ログを生成します。応用性の高いCUIツールとして、他の解析ツールや自動化パイプラインとの連携も容易です。遠征先での利便性と、緻密なデータ解析を両立したい天文ファンへ最適なプラットフォームを提供します。
...
* **Standardized Logging**: すべてのショット情報を **JSON Spec v1.5.0** に準拠した形式で保存。
...
* **Software Version**: v14.2.0
* **JSON Log Spec**: [v1.5.0](./OFS_json_spec.md)
