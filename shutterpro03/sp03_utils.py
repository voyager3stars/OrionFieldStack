...
def print_help(config):
    ...
    def p_opt(key, desc, current):
        print(f"  {C_KEY}{key:<22}{C_RESET}: {desc:<35}{C_VAL}[Current: {current}]{C_RESET}")
    ctx = config.get('CONTEXT', {})
    eqp = config.get('EQUIPMENT', {})
    sys_conf = config if 'SAVE_DIR' in config else config.get('SYSTEM', {})
    print(f" {C_HEAD}[CONTEXT]{C_RESET}")
    p_opt("obj= / objective=",  "Target name / Object ID",         ctx.get('objective', 'N/A'))
    p_opt("sess= / session=",   "Session ID",                      ctx.get('session', 'def'))
    p_opt("t= / type=",         "Frame type (light, dark...)",     ctx.get('frame_type', 'test'))
    print(f"\n {C_HEAD}[EQUIPMENT]{C_RESET}")
    p_opt("tel= / telescope=",  "Telescope / OTA",                 eqp.get('telescope', 'N/A'))
    p_opt("cam= / camera=",     "Imaging Camera",                  eqp.get('camera', 'N/A'))
    p_opt("f= / focal=",        "Focal Length (mm)",               eqp.get('focal_length_mm', 0))
    p_opt("opt= / optics=",     "Optics / Reducer",                eqp.get('optics', 'N/A'))
    p_opt("fil= / filter=",     "Filter name",                     eqp.get('filter', 'N/A'))
    print(f"\n {C_HEAD}[SYSTEM]{C_RESET}")
    p_opt("dir=",               "Save Directory",                  sys_conf.get('SAVE_DIR', ''))
    p_opt("log_dest=",          "Log Destination (s2cur/s2save)",  sys_conf.get('LOG_DEST', 's2cur'))
    p_opt("display=",           "Log verbosity (full/simple/off)", sys_conf.get('DISPLAY_MODE', 'full'))
    print(f"\n {C_HEAD}[INDI]{C_RESET}")
    p_opt("mnt= / mount=",      "INDI Mount device name",          config.get('INDI_MOUNT', 'N/A'))
    p_opt("wth= / weather=",    "INDI Weather device name",        config.get('INDI_WEATHER', 'N/A'))
    print("")
