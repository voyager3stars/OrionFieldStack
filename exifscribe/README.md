...
ExifScribeは、撮影されたDNGファイルからEXIF情報を抽出し、`shutter_log.json` の該当レコードを補完または更新するためのツールです。OrionFieldStack v1.6.3 仕様に準拠し、記録漏れや情報の不一致を効率的に解消します。
...
### v1.6.1 (2026-06-20)
* **DNG画像サイズ（Width/Height）検出の修正**:
  DNG(RAW)画像において、サムネイル解像度（160x120）が優先されて記録される不具合を修正しました。`Image SubIFDs` 内の解像度タグ（Tag 256/257）をバイナリパースして最大寸法（フルサイズ）を正確に取得する処理を追加。
* **--force オプションの挙動改善**:
  `--force` 指定時、画像のミスマッチが発生しても対話プロンプト（Overwrite選択など）を出さず、最優先で強制上書き（非対話）するように改善。


* **v1.1.2** (2026-06-28)
  - **OrionFieldStack JSON Spec v1.6.3 準拠**: 内部仕様バージョンを 1.6.3 に更新。StarFlux などの他のツールが追加したJSONキーを破壊せず保持する仕様であることを再確認。
